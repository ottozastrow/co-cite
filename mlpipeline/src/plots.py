...
def plot_accs_per_occurrence(samples_table, columns) -> None:
    if len(samples_table) == 0:
        print("WARNING: empty table, can not plot accs per occurrence")
    else:
        df = pd.DataFrame(samples_table, columns=columns)
